...
class DmarcvalidatorValidationException implements Exception {
  final List<String> errors;

  DmarcvalidatorValidationException(this.errors);

  @override
  String toString() => 'DmarcvalidatorValidationException: ${errors.join("; ")}';
}
...
/// DMARC Validator API client.
///
/// For more information, visit: https://dmarcvalidator.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [domain] (required) - The domain to validate the DMARC record for
class DmarcvalidatorClient {
  final String apiKey;
  ...
  DmarcvalidatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/dmarcvalidator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DmarcvalidatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DmarcvalidatorValidationException(errors);
    }
  }

  /// Execute a request to the DMARC Validator API.
  ///
  /// Parameters are validated before sending the request.
  Future<DmarcvalidatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DmarcvalidatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DmarcvalidatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DmarcvalidatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DmarcvalidatorException('Resource not found');
      } else {
        throw DmarcvalidatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DmarcvalidatorException) rethrow;
      throw DmarcvalidatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the DMARC Validator API client.
class DmarcvalidatorException implements Exception {
  final String message;

  DmarcvalidatorException(this.message);

  @override
  String toString() => 'DmarcvalidatorException: $message';
}
